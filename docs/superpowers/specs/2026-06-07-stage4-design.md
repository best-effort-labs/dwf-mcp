...
- **UART** is an exception: the UART engine supports simultaneous TX and RX, enabling loopback self-test where the AD3 transmits and sniffs on separate pins.
...
- `src/dwf_mcp/backends/pydwf_backend.py` — add `i2c_spy_start`, `i2c_spy_status`, `uart_sniff`, `can_sniff`
...
- Configures UART engine with `rxSet(rx_idx)` only (no `txSet`), same init sequence as `uart_configure`.
- Polls `uart.rx(256)` in a drain loop until deadline.
- Returns `list[tuple[float, bytes, bool]]` — `(timestamp_s, data, parity_error)`.

Because the UART engine supports full-duplex, a test setup with the AD3 transmitting on DIO0 (Jumperless DIO0→DIO1) and `sniff.uart` on DIO1 is feasible and does not conflict at the hardware level.

### sniff.can — hardware RX-only

New backend method `can_sniff(rx_pin_idx, bitrate, duration_s, poll_interval_s)`:
- Configures CAN engine with `rxSet(rx_idx)` only (no `txSet`).
...
1. Claims pins as DigitalIn observer (see Allocator Extension below).
...
4. Passes the raw sample array to `SpiDecoder`.
...
**Inputs:** numpy array of shape `(n_samples, 16)` with uint8 0/1 values per bit; `pin_map` dict mapping `"clk"`, `"mosi"`, `"miso"` (optional), `"cs"` (optional) to column indices; mode (CPOL/CPHA 0–3), bit_order, word_size.
...
- `allocator.claim_observe(instrument, pins)` — records a read-only DigitalIn observation claim.
- Observer claims **do not** conflict with write claims (DigitalOut) on the same pins — this is the hardware reality: DigitalOut drives the pin, DigitalIn observes it, no conflict.
- Observer claims **do** conflict with other observer claims on the same pins — there is only one DigitalIn block; two concurrent DigitalIn configurations cannot coexist.
- Observer claims also conflict with any exclusive claim that configures the same DigitalIn resource (e.g. `logic.configure` or `logic.record_start` already holding the DigitalIn block).
- `sniff.spi` uses `claim_observe`; all other sniff tools use the existing `claim` (exclusive).
...
- Pattern drives CLK on DIO0 with custom bit pattern representing known SPI frames; MOSI on DIO1.
- Jumperless loops DIO1→DIO2 (MOSI→MISO).
- `sniff.spi` captures on DIO0/DIO1/DIO2 using `claim_observe`.
- Verify word count and round-trip MOSI=MISO.
- Run `decoder.spi` on a separately saved npz; verify identical output.

**`test_sniff_uart_hardware.py`** — automated using UART full-duplex loopback:
- AD3 transmits on DIO0; Jumperless loops DIO0→DIO1.
- `sniff.uart(rx_pin="dio1", ...)` claims DIO1 exclusively (no engine conflict — TX and RX are separate pin assignments within the same UART engine).
- Verify received frames match transmitted bytes.
