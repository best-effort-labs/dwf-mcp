"""Pure single-tone-sweep DSP shared across the frequency-domain analyzer trio
(Bode, Impedance) — no hardware, no I/O.
...
QF_LOW_VIN_RMS = 1 << 8
QF_CLIPPED = 1 << 9
QF_LOW_DRIVE = 1 << 10        # |V_total - V_dut| (current) below threshold — high-Z degeneration
QF_LOW_DUT_VOLTAGE = 1 << 11  # |V_dut| below threshold — low-Z degeneration
QF_REF_MISMATCH = 1 << 12     # |Z|/R_ref grossly off — supplementary guidance
...
QF_NAMES: dict[int, str] = {
    ...
    QF_CLIPPED: "clipped",
    QF_LOW_DRIVE: "low_drive",
    QF_LOW_DUT_VOLTAGE: "low_dut_voltage",
    QF_REF_MISMATCH: "ref_mismatch",
}
