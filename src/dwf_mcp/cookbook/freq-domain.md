...
- **THD** (total harmonic distortion): `formulae.thd(result, fundamental_hz, n_harmonics=5)` — returns `sqrt(Σ V_n²) / V_1`. Uses nearest-bin amplitudes from the `SpectrumResult`; harmonics beyond Nyquist are skipped. Status: **verified (fake-oracle + hardware: a 1 kHz square-wave loopback on AD3 `210415BB5F2A` measured THD = 0.4268 over 9 harmonics — ideal odd-harmonic square is ≈0.483, pulled down by combined AWG/scope bandwidth; `tests/hardware/test_cookbook_formulae_hardware.py::test_square_wave_thd_matches_theory`)**.
